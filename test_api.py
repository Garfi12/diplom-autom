...
def test_add_product_to_cart():
    """
                        ...
                        Проверяет, успешен ли запрос на
...
def test_edit_cart():
    ...
    # Создаем экземпляр класса API для редактирования корзины
    ...
    update_cart_response = (200,
                            {'products': [{'id': 141579548, 'quantity': 2}]})
    # Пример ответа
...
def test_delete_product_from_cart():
    ...
    # Получаем содержимое корзины, чтобы убедиться,
    # что добавленный товар есть в ней
    ...
    with allure.step("Проверить статус запроса на "
...
@allure.feature("Тестирование API интернет-магазина")
@allure.story("Запрос на добавление товара в "
              "корзину используя неправильный метод (PATCH вместо POST)")
def test_wrong_add_request():
    """
                    Тест для некорректного добавления продукта в корзину.
                    Проверяет, правильный ли статус-код
                     возвращается для запроса с ошибкой.
    """
...
def test_add_product_to_cart_with_empty_body():
    ...
    Проверяет, как API реагирует на пустой запрос,
    ожидая, что сервер вернет статус ошибки.
    """
    with allure.step("Отправить пустой запрос в корзину"):
        ...
        # Вызов метода с пустым телом

    with ((allure.step("Проверить статус запроса"))):
        # Проверяем, что сервер возвращает статус 422
        assert status_code == 422, ("Ожидается статус "
