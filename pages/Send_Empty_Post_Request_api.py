...
from constants import bearer_token, User_Agent, User_ID
...
class EmptyPostRequest:
    """
                    ...
                    Этот класс предназначен для отправки запросов
                    к API, связанным с добавлением продуктов в корзину.
    """
    ...
    def add_product_to_cart_with_empty_body(self) -> None:
        """
                Отправляет POST-запрос с пустым телом
                для добавления продукта в корзину.
                :return: Статус код ответа от сервера (int).
                Ожидается, что в случае пустого
                тела запроса сервер вернет код 422.
        """
