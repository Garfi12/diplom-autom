...
import json
import allure
from constants import API1_url, bearer_token, User_Agent, User_ID


@allure.description("Тестирование добавления товара в "
                    "корзину на сайте Читай-город.")
class AddToCartAPI:
    ...
    def __init__(self, url):
        ...
        self.headers = {
            ...
            'User-Agent': User_Agent,  # Добавляем User-Agent
            'User-ID': User_ID  # Добавьте идентификатор пользователя
        }

    def add_product_to_cart(self, product_id: int, item_list_name: str) -> int:
        """             Добавляет товар в корзину и возвращает статус-код
        ответа.

                        Args:
                            product_id (int): ID товара для добавления.
                            item_list_name (str): Имя списка,
                            к которому принадлежит товар.

                        Returns:
                            int: Статус-код ответа от сервера (например,
                            200 для успешного добавления).
        """
